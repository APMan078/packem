...
namespace Packem.Mobile.Modules.Receipts
{
    ...
    public class ReceiptLotLookupViewModel : BaseViewModel
    {
        #region "Variables"
        ...
        private IEnumerable<LotViewModel> DtoToVm(IEnumerable<LotLookupGetModel> dto)
        {
        ...
        private async Task Search(string searchText)
        {
            try
            {
                ...
                var result = await _lotService
                    .GetLotLookupByItemIdDeviceAsync(state.DeviceState, ItemId, searchText);
                ...
                await _unauthorizedService
                    .UnauthorizedWorkflowIfNotAuthenticated(result, async () =>
                    {
                        ...
                        Device.BeginInvokeOnMainThread(() => {
                            Lots.Clear();
                            Lots.AddRange(DtoToVm(result.Response));
                        });
                    });
            }
            finally
        ...
        private async Task GridTap(GridSelectionChangedEventArgs e)
        {
            try
            {
                ...
                var selectedItem = e.AddedItems[0] as LotViewModel;
        ...
        public static event Action<LotViewModel> SelectedItem
        {
        ...
        public ReceiptLotLookupViewModel(INavigationService navigationService,
            ...
            IUnauthorizedService unauthorizedService)
        {
            ...
            BindingBase.EnableCollectionSynchronization(Lots, null, ObservableCollectionCallback);
        }
        ...
        public override async Task InitializeAsync()
        {
            try
            {
                ...
                var state = JsonConvert.DeserializeObject<MobileState>(stateJson);
                var result = await _lotService
                    .GetLotLookupByItemIdDeviceAsync(state.DeviceState, ItemId, null);

                Lots.CollectionChanged += (sender, args) =>
                {
                    if (Lots.Count() > 0)
                        HasRecord = true;
                    else
