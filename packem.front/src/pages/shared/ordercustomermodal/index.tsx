...
export default React.memo(
  ({ callBack, edit, orderCustomer }: OrderCustomerModalProps) => {
    ...
    const onSaveNewOrderCustomer = async () => {
      schema
        ...
        .then(async () => {
          try {
            if (edit) {
              await editOrderCustomer(form);
              snackActions.success(`Successfully edited customer ${form.name}`);
            } else {
              await createOrderCustomer(form);
    ...
    }, [currentLocationAndFacility, orderCustomer, edit]);
